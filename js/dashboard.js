...
// ✅ FIXED: Load sheet music download links with deduplication
...
try {
  ...
  if (!purchases || purchases.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        <div class="empty-state-icon">📄</div>
        <p>No sheet music yet. Purchase a song to unlock its PDF.</p>
      </div>
    `;
    return;
  }
  ...
  const container = document.getElementById('sheetMusicList');
  if (container) container.innerHTML = `<p style="color:red;">Error loading sheet music</p>`;
}
}
