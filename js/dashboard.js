...
function setAvatar(src) {
  ...
  // Also update banner avatar
  const bannerImg = document.getElementById('bannerAvatar');
  if (bannerImg) {
    bannerImg.src = src || DEFAULT_AVATAR;
    bannerImg.onerror = () => { bannerImg.src = DEFAULT_AVATAR; };
  }
}
...
async function initDashboard() {
 ...
 const displayName = profile?.display_name || user.email.split('@')[0];
    ...
    setupBonusVideoModal(user.id); // ✅ Setup bonus video modal
...
// ─────────────────────────────────────────────
// BONUS VIDEO MODAL: setup and upload
// ─────────────────────────────────────────────
function setupBonusVideoModal(userId) {
  ...
  uploadBtn.addEventListener('click', async () => {
    ...
    if (file.size > maxSize) {
      feedback.textContent = 'Video must be under 100MB.';
      feedback.className = 'modal-feedback error';
      return;
    }
    ...
    try {
      ...
      const filePath = `${userId}/bonus-video-${Date.now()}.${ext}`;
      ...
      // Add 1 credit to user_credits using RPC function
      const { error: creditError } = await window.supabase.rpc('add_user_credit', { 
        p_user_id: userId, 
        p_amount: 1 
      });
      ...
      if (creditError) {
        ...
        // Don't throw - video is uploaded, we'll add credit manually if needed
      }

      feedback.textContent = '🎉 Success! 1 free song credit added!';
...
// NEW: Setup avatar removal
function setupAvatarRemoval(userId) {
  const removeBtn = document.getElementById('removeAvatarBtn');
  if (!removeBtn) return;

  removeBtn.addEventListener('click', async () => {
    const confirmed = confirm('Reset your profile photo to default?');
    if (!confirmed) return;
...
async function loadStats(userId) {
  try {
    ...
    // ✅ FIXED: Get book bonus credits (from user_credits table) with .maybeSingle()
...
// ✅ UPDATED: Load book verification status with bonus video visibility
async function loadBookStatus(userId) {
  try {
    ...
    // ✅ Show/hide bonus video section based on claim status
    const bonusSection = document.getElementById('bonusVideoSection');
    if (bonusSection) {
      // Show if user is a book owner AND hasn't claimed the bonus yet
    ...
    // ✅ FIXED: Also check for book bonus credits with .maybeSingle()
...
async function loadSongs(userId) {
  try {
    ...
    container.innerHTML = purchases.map(purchase => {
      ...
      const songSlug = purchase.song_slug;
    ...
    <button class="download-pdf-btn" onclick="downloadSheetMusic('${songId}', '${songId}')" title="Download sheet music PDF">
  📄 PDF
</button>
...
async function updateProgress(songId, newStatus) {
  try {
    ...
    buttons.forEach(btn => {
      ...
      const key = btn.textContent.trim().toLowerCase().replace(/\s+/g, '_');
      if (key === newStatus) btn.classList.add('active');
    });
...
const SUB_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx5cXB4Y2lsbmlxenVyZXZldGFlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk1NDAyMTQsImV4cCI6MjA4NTExNjIxNH0.40ZbAatkMBFHacQGCpiNYpjcKQoZik-Xvqx3bG46x7c';
